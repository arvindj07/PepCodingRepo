...
(async function () {
  try {
    ...
    let videosViewsArr = await page.evaluate(getVideosViews, videoNdViewSelector);
    ...
    let videosCount = videos.split(" ")[0]; // to get only count
    ...
    while (videosCount - 50 > pCurrentVideoCount) {
...
async function scrollToBottom(page,title){
  ...
  function getLengthConsoleFn(title){
    ...
    return titleElemArr.length; // return the whole length
  }
